...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # output projection
        self.c_proj = nn.Linear(config.n_embd, config.n_embd)  # Fix typo from == to =
        # regularization
    ...
    def forward(self, x):
        B, T, C = x.size()  # batch size, sequence length, embedding dimensionally (n_embd)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        # init a huggingface / tansformer model
        model_hf = GPT2LMHeadModel.from_pretrained(model_type)
        sd_hf = model_hf.state_dict()

        # copy while ensuring all of the paramters are aligned and match in names and shapes
